//Дан список из класса телефон, где каждая запись характеризуется номером телефона, фио владельца, оператором подключения. Необходимо спомощью элемента коллекции словарь определить частоту использования оператора связи
using System;
class Telephon
{
